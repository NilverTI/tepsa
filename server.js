...
const server = http.createServer(async (request, response) => {
  ...
  if (url.pathname === "/api/ps-ranking") {
    await handlePSRanking(request, response, url);
    return;
  }

  if (url.pathname === "/api/ranking") {
    await handleRankingHistory(request, response, url);
    return;
  }
...
async function handlePSRanking(request, response, url) {
  return handleRankingHistory(request, response, url);
}
...
async function handleRankingHistory(request, response, url) {
  const rankingHandler = require("./api/ranking");
  ...
  const fakeRes = {
    ...
    json(data) {
      sendJson(response, data, response.statusCode || 200);
    }
  };
  await rankingHandler(fakeReq, fakeRes);
}
